...
<nav class="flex flex-col space-y-6">
  ...
  <div class="flex-1 p-6">
    <!-- Toggle Button -->
    <h1 class="text-2xl font-bold text-[#AF1740]">Kategori</h1>
          ...
          <tbody class="text-sm text-gray-800">
            @forelse ($kategori as $category)
              <tr class="border-b border-gray-200 hover:bg-gray-50">
                <td class="py-3">{{ $category->id_kategori }}</td>
                <td class="py-3">{{ $category->nama_kategori }}</td>
                <td class="py-3">
                  <a href="{{ route('admin.categories.edit', $category->id_kategori) }}" class="text-blue-600 hover:underline">Edit</a>
            ...
            @empty
              <tr>
                <td colspan="3" class="py-3 text-center text-gray-500">No categories found.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
